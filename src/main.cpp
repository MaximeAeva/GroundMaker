...
int main()
{
    ...
    // --------------------
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Ground Maker", NULL, NULL);
    if (window == NULL)
    {
    ...
    int number = 3;//Number of square on a side (total side points = (2^dim + 1)*number)
    int filter = 3;//Side size of the median filter
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(mappedVert[0]) * 6*pow(dimension, 2), mappedVert, GL_DYNAMIC_DRAW);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(mappedIdx[0])* 2*3*pow(dimension-1, 2), mappedIdx, GL_DYNAMIC_DRAW);
    // position attribute
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glDrawElements(GL_TRIANGLES, int(6*pow(dimension-1, 2)), GL_UNSIGNED_INT, 0);
